...
<!-- Content Header (Page header) -->
    <section class="content-header">
      <h1>
        <a href="{{url("approval_1_regtt")}}" class="btn btn-danger btn-sm"><i class="fa fa-arrow-left"></i> kembali</a> 
        {{-- Data Registrasi Tenaga Ahli - Tahap {{count($regtas) > 0 ? $regtas[0]->tahap1 : "-"}} --}}
        {{--  <small>it all starts here</small>  --}}
      </h1>
      <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
        <li><a href="{{url("approval_1_regtt")}}">Approval Tenaga Trampil</a></li>
                    ...
                    <tbody>
                      @foreach($results as $k => $result)
                        <tr>
                          ...
                          <td>
                              {{-- <a data-team="{{($role == 1 || $role == 2) ? "1" : "0"}}"  href="{{url("approval_1_regtt/" . $result->ID_Asosiasi_Profesi . "/approve") . '?' . http_build_query($result)}}" class="btn btn-primary btn-xs approve">Approve</a> --}}
                              <a href="{{url("approval_1_regtt/" . $result->ID_Asosiasi_Profesi . "/approve") . '?' . http_build_query($result)}}" class="btn btn-primary btn-xs">Approve</a>
                          </td>
                        </tr>
                      @endforeach
                    </tbody>
