...
<div class="doc">
  ...
  <div class="nobreak">
    ...
    <table class="rowdata" cellpadding="0" cellspacing="0">
      ...
      @foreach($regta as $i => $r)
        <tr>
          <td class="center">{{$i + 1}}</td>
          <td>{{$r->ID_Personal}}</td>
          <td>{{$r->personal->Nama}}</td>
          <td>{{$r->ID_Sub_Bidang}}</td>
          <td>{{$r->asosiasi->nama}}</td>
          <td>{{$r->kualifikasi->deskripsi_ahli}}</td>
          <td>{{$r->id_permohonan == "1" ? "Baru" : ($r->id_permohonan == "2" ? "Perpanjangan" : "Perubahan") }}</td>
        </tr>
      @endforeach
    </table>
  </div>
